		...
		return (
			<div className="dropdown">
				...
				<select 
					...
					>
					{do{
						if(isFirstKeyString(options)){
							// don't need a return in an if statement?
							Object.keys(options).map(function(strOptionKey){
								return (											
									<option value={strOptionKey}>
							...
							// don't need a return in an if statement?
							options.map(function(objOptGroup){
								return Object.keys(objOptGroup).map(function(strOptGroupKey){
									return (											
										<optgroup label={strOptGroupKey}>
											{
												Object.keys(objOptGroup[strOptGroupKey]).map(function(strOptionKey){
													return (											
														<option value={strOptionKey}>{objOptGroup[strOptGroupKey][strOptionKey]}</option>
													)
												})
											}
										</optgroup>
